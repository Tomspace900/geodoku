...
Pays partageant une frontière terrestre avec **la Russie**.
...
`content/constraints/derivations.ts` : `borders` contient `RUS`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
