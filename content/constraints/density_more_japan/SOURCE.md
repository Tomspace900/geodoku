---
constraint_id: density_more_japan
status: active
...
Pays dont la densité de population est supérieure à celle du Japon.
...
`content/constraints/derivations.ts` : `population / areaKm2` > densité de `factsOf("JPN")` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).

## Cas limites

La densité brute ignore les zones inhabitables : un pays majoritairement désertique ou montagneux peut surprendre. Un pays très proche du seuil bascule selon le millésime de la source. La dérivation tranche sur la donnée figée du snapshot (`FACTS_SNAPSHOT.date`) ; une regen sur données fraîches peut faire basculer un cas limite. Regen du 2026-08-28 : la **Grenade** sort (densité ≈ 317 hab/km² < Japon ≈ 325).
