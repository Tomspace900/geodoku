---
constraint_id: continent_europe
status: active
...
Pays dont le continent Geodoku est **Europe**.
...
`content/constraints/derivations.ts` : `continent` == `europe`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
