...
Pays dont le continent Geodoku est **Asie**.
...
`content/constraints/derivations.ts` : `continent` == `asia`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
