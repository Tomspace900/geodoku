---
constraint_id: continent_south_america
status: active
...
Pays dont le continent Geodoku est **Amérique du Sud**.
...
`content/constraints/derivations.ts` : `continent` == `south_america`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
