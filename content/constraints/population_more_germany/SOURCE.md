---
constraint_id: population_more_germany
status: active
...
Pays dont la population est supérieure à celle de l'Allemagne.
...
`content/constraints/derivations.ts` : `population` > `factsOf("DEU").population` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
