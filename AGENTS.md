...
content/                  # snapshot de contenu versionné, TERMINAL (n'importe rien de src/convex)
  ...
  sources.ts               # registre des sources externes (nom, URL, millésime), partagé par les about.ts
  countries/              # catalog (identité) + countryCodes + facts + popularity (générés, datés) + SOURCE.md
  constraints/            # index (registre typé) + derivations (prédicats) + abouts.ts (registre "à propos") + SOURCES.md + <id>/{answers.ts, about.ts, SOURCE.md}
...
- Chaque contrainte active/archivée porte aussi un `about.ts` (source(s) + millésime, `basis` `source`/`convention`, clarification ≤ 160 caractères sans nom de pays hors libellé) — c'est ce texte, pas le `SOURCE.md`, qu'affiche le toast joueur au tap sur un en-tête de contrainte. Registre séparé [`content/constraints/abouts.ts`](content/constraints/abouts.ts) : **jamais** dans `index.ts`, que Convex importe — les textes joueur n'ont rien à faire côté serveur. Sources externes centralisées dans [`content/sources.ts`](content/sources.ts). Gardé par `validateConstraintAbouts` (`pnpm check:content`).
...
- [`content/countries/factProvenance.ts`](content/countries/factProvenance.ts) déclare, pour chaque **ligne affichée de la fiche pays** (`SheetRowId`, pas `keyof CountryFacts`), sa provenance (`FactProvenanceSources`, même union `basis: "source"/"convention"` + `SourceId[]` que `about.ts` des contraintes) — un champ composite (`physicalFeatures`, `geoTags`) se décompose en plusieurs lignes, chacune avec sa propre source (façade → IHO, équateur → Natural Earth, milieu → MODIS), et `PRODUCTION_PROVENANCE` porte à part la provenance par produit affiché de `productionRanks`. Consommé par `buildCountrySheet` ([`src/features/countries/logic/countrySheet.ts`](src/features/countries/logic/countrySheet.ts)), avec `mergeBasis` (convention l'emporte) pour les lignes composites. Une ligne qui recoupe une contrainte du lot 1 reprend exactement la même provenance que son `about.ts`, vérifié par un test unitaire ([`content/countries/__tests__/factProvenance.test.ts`](content/countries/__tests__/factProvenance.test.ts)). `check:content` étend la garde « source référencée » du lot 1 à ce fichier ([`validateFactProvenance.ts`](scripts/content/validateFactProvenance.ts)).
- La fiche pays est chargée en **lazy** : seul [`countrySheetData.ts`](src/features/countries/logic/countrySheetData.ts) importe `content/countries/facts.ts` côté joueur (préchargé au montage de la grille solution, jamais avant). `check:bundle` ([`check-bundle-size.ts`](scripts/ci/check-bundle-size.ts)) vérifie explicitement que `facts.ts` n'atteint jamais le graphe initial, en plus du chunk dédié listé dans `LAZY_FEATURE_MODULES`.
...
- **Récoltes réseau** (`scripts/countries/harvest/`, ex. `pnpm harvest:capitals` pour les libellés fr/en des capitales, Wikidata) : un script par source, séparé de `build-countries`, qui écrit un dataset de `scripts/countries/data/` relu en diff puis committé ; `build:countries` le fusionne hors-ligne. Le dataset porte un bloc `overrides` curé (motif obligatoire), préservé à chaque récolte. La récolte **traduit** une liste qui reste celle de REST Countries : jamais d'ajout ni de retrait de capitale, jamais de repli silencieux sur l'anglais ; une nouvelle récolte imprime le millésime à reporter à la main dans `content/sources.ts`, que `check:content` compare exactement à `harvestedAt` — [`validateCapitalLabels`](scripts/countries/validateDatasetFacts.ts) (`check:content`) échoue sur un libellé obsolète ou une entrée orpheline.
...
pnpm check:content               # cohérence content/ (job quality + pre-commit) : obsolescence des listes + fraîcheur des faits dérivés + cohérence souveraineté + seam terminal + provenance SOURCE.md + sources des contraintes (about.ts)
pnpm analyze:pool
...
| Sources | `constraint_source_viewed` (`grid_date`, `mode`: `daily`/`training`, `surface`: toujours `playing`, `constraint_id`) — tap sur un en-tête de contrainte **en jeu uniquement** (toast de source, [`useConstraintSourceToast`](src/features/game/hooks/useConstraintSourceToast.ts)) ; sur la grille solution, l'en-tête ouvre le Drawer de contrainte à la place (`solution_constraint_opened`, ci-dessous) — `surface` reste dans l'event pour ne pas casser les groupements existants, bien qu'il ne prenne plus qu'une seule valeur |
| Fiche pays | `solution_cell_opened` (`grid_date`, `mode`, `cell`, `answer_count`) et `solution_constraint_opened` (`grid_date`, `mode`, `constraint_id`, `answer_count`) — tap sur une case ou un en-tête de la grille solution, qui ouvre le Drawer partagé ([`useSolutionDrawer`](src/features/game/hooks/useSolutionDrawer.ts)) ; `country_sheet_opened` (`grid_date`, `mode`, `country_code`, `origin`: `cell`/`constraint`, plus `cell` ou `constraint_id` selon le cas) — tap sur une réponse du Drawer, qui ouvre sa fiche. Les trois events sont **partagés** daily/entraînement, distingués par `mode`, jamais émis en cours de partie (la grille solution n'existe qu'une fois `status !== "playing"`) |
| Entraînement | `archive_opened`, `training_started` (`grid_date`, `resumed`, `restart`), `training_completed` (`grid_date`, `outcome`, `filled_cells`, `failed_attempts`), `training_result_viewed` (`grid_date`, `outcome`, `score_total`/`score_grid`/`score_rarity` sur 900, `failed_attempts`) — **noms distincts des events daily** pour ne pas corrompre les groupements existants. Corollaire : le mode entraînement n'émet **aucun** event daily, `cell_opened` compris (il alimente le tunnel du quotidien) ; `game_started`/`game_completed`/`result_screen_viewed`/`cell_opened` restent daily-only. `scoring_info_opened` et `constraint_source_viewed` sont partagés, distingués par leur prop `mode`/`source`. |
