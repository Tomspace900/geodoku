...
| Sources | `constraint_source_viewed` (`grid_date`, `mode`: `daily`/`training`, `surface`: `playing`/`solution`, `constraint_id`) — tap sur un en-tête de contrainte (toast de source) ; event **partagé**, distingué par `mode`, sur la grille de jeu comme sur la grille solution |
| Fiche pays | `solution_cell_opened` (`grid_date`, `mode`, `cell`, `answer_count`) et `country_sheet_opened` (`grid_date`, `mode`, `cell`, `country_code`) — tap sur une case de la grille solution puis sur une réponse de son Drawer ([`useSolutionCellDrawer`](src/features/game/hooks/useSolutionCellDrawer.ts)) ; events **partagés** daily/entraînement, distingués par `mode`, jamais émis en cours de partie (la grille solution n'existe qu'une fois `status !== "playing"`) |
