...
États membres de **l'Union européenne** retenus par la convention Geodoku.
...
`content/constraints/derivations.ts` : `memberships` contient `eu`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
