...
// Dérivé de la constante plutôt qu'en dur : la fenêtre de 7 jours est la règle
...
describe("isChangelogNewBadgeVisible", () => {
  it("est visible dans les 7 jours après la date de release (UTC)", () => {
    expect(isChangelogNewBadgeVisible(at(0))).toBe(true);
    expect(isChangelogNewBadgeVisible(at(12 * HOUR_MS))).toBe(true);
    expect(isChangelogNewBadgeVisible(at(WINDOW_MS - 1))).toBe(true);
  });

  it("disparaît après 7 jours", () => {
    expect(isChangelogNewBadgeVisible(at(WINDOW_MS))).toBe(false);
  });
