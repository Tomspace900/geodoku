...
Pays dont la population est supérieure à celle du Canada.
...
`content/constraints/derivations.ts` : `population` > `factsOf("CAN").population` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
