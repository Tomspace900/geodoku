---
constraint_id: area_larger_mexico
status: active
...
Pays dont la superficie est supérieure à celle du Mexique.
...
`content/constraints/derivations.ts` : `areaKm2` > `factsOf("MEX").areaKm2` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
