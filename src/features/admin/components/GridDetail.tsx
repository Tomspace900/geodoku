import { isFutureDate } from "@/features/admin/logic/cellMetrics";
import {
  difficultyStars,
...
import { CellMetricsList } from "./CellMetricsList";
...
export function GridDetail({ grid, selectedDate, token }: Props) {
  ...
  const showCellMetrics = grid !== null && !isFutureDate(grid.date);
  const cellMetrics = useQuery(
    api.grids.getGridCellMetrics,
    showCellMetrics ? { date: grid.date, adminToken: token } : "skip",
  );
