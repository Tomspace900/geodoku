---
constraint_id: continent_africa
status: active
...
Pays dont le continent Geodoku est **Afrique**.
...
`content/constraints/derivations.ts` : `continent` == `africa`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
