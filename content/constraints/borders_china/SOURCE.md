...
Pays partageant une frontière terrestre avec **la Chine**.
...
`content/constraints/derivations.ts` : `borders` contient `CHN`. `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
