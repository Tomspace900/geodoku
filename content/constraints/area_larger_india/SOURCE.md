---
constraint_id: area_larger_india
status: active
...
Pays dont la superficie est supérieure à celle de l'Inde.
...
`content/constraints/derivations.ts` : `areaKm2` > `factsOf("IND").areaKm2` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
