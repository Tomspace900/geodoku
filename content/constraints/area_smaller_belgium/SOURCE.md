---
constraint_id: area_smaller_belgium
status: active
...
Pays dont la superficie est inférieure à celle de la Belgique.
...
`content/constraints/derivations.ts` : `areaKm2` < `factsOf("BEL").areaKm2` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
