...
- [`content/countries/factProvenance.ts`](content/countries/factProvenance.ts) déclare, pour chaque champ de `CountryFacts`, sa provenance (`FactProvenanceSources`, même union `basis: "source"/"convention"` + `SourceId[]` que `about.ts` des contraintes) — consommé par `buildCountrySheet` ([`src/features/countries/logic/countrySheet.ts`](src/features/countries/logic/countrySheet.ts)) pour attribuer chaque ligne de la fiche pays, avec `mergeBasis` (convention l'emporte) pour les lignes composites. `check:content` étend la garde « source référencée » du lot 1 à ce fichier ([`validateFactProvenance.ts`](scripts/content/validateFactProvenance.ts)).
