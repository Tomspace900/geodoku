---
constraint_id: density_less_canada
status: active
...
Pays dont la densité de population est inférieure à celle du Canada.
...
`content/constraints/derivations.ts` : `population / areaKm2` < densité de `factsOf("CAN")` (valeur live du repère). `pnpm build:answers` matérialise la liste ISO3 dans `answers.ts` (relue en diff, gardée par `pnpm check:content`).
